...
I require that the tumor and normal are measured on the same CpG sites, to control for different sequencing depth of some tumors.  Samples are processed individually and CpGs are merged for comparisons of PWDs before calling intervals. 
...
It's hard to know if the clusters near 50000 bps in width are problematic.

In normal I drop those with > 10000 bps.  
...
Should I add another filter? I can drop 25\% of intervals if I require > 9 CpGs (~20,000 remaining).

```{r}
#nn.dt.f <- nn.dt[ n_CpG > 9,]
ggplot(nn.dt, aes( x = n_CpG, y = width)) +
    geom_point()
```
