---
title: "CTCF motif regions of interest"
author: "ks"
date: "2024-11-22"
...
We want to compare DNA methylation conservation in the colon cancer-specific CTCF binding sites vs control sites. Possible controls:   
1. CTCF binding in BRCA but not COAD  
2. FIMO-predicted CTCF, but not COAD
...
### 3 JASPAR Motifs (MA0139.1 + 2 others)
...
If we filter more aggresively (p-value < 1e-5), fewer than 50% as many COAD-specific binding sites will be validated (<50% as many) so I'll keep the standard p < 1e-4 cutoff.
...
### Annotate interval as being in COAD, BRCA and FIMO
```{r findO}
values(ctcf_motifs)$pred_type <- values(ctcf_motifs)$cancer_type

fO <- findOverlaps(ctcf_motifs,ctcf.all.noovrlp)
length(unique(queryHits(fO)))
values(ctcf_motifs)$pred_type[queryHits(fO)] <-
                  paste0(values(ctcf_motifs)$cancer_type[queryHits(fO)],"+FIMO")
table(values(ctcf_motifs)$cancer_type,values(ctcf_motifs)$pred_type)
```

Add FIMO-predicted only (not in COAD & not in BRCA). I'm going to use the ma0139.1 motif only because there are tons of predicted sites from which to choose.  I don't need to include intervals from other motifs.

```{r}
FIMOma0139.1_only_ctcf <- subsetByOverlaps(ctcf.ma0139.1.noovlp,ctcf_motifs, invert=TRUE)
...
```{r combine-gr}
ctcf_motifs_fimo <- c(ctcf_motifs,FIMOma0139.1_only_ctcf)
