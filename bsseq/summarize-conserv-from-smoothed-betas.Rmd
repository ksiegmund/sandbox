...
nn.lst <- c("JN","IN","EN")
tumor.lst <- c("MA","MB","HA","HB","IA","IB","EA","EB")
...
dtc <- merge(
            tumorpair.dt_list[[1]][, .(seqnames, start, end, 
                    pair1.pwd = pwd,
...
ndt <- names(dtc)  
ndt <- sub("pair1",names(tumorpair.dt_list[1]),ndt)
ndt <- sub("pair2",names(nnpair.dt_list[1]),ndt)
names(dtc) <- ndt
```
...
ndt <- names(dtc4)  
ndt <- sub("newpair",names(tumorpair.dt_list[1]),ndt)
names(dtc4) <- ndt
rm(dtc3)
...
dtc5 <- merge(
            dtc4,
            tumorpair.dt_list[[2]][, .(seqnames, start, end, 
                    newpair.pwd = pwd, 
                    newpair.avgdepth_tot = avgdepth_tot,
                    newpair.avgb = avgb)],
            by.x = c("seqnames", "start", "end"),
...
ndt <- names(dtc5)  
ndt <- sub("newpair",names(tumorpair.dt_list[2]),ndt)
names(dtc5) <- ndt
rm(dtc4)
...
dtc6 <- merge(
            dtc5,
            tumorpair.dt_list[[3]][, .(seqnames, start, end, 
                    newpair.pwd = pwd, 
                    newpair.avgdepth_tot = avgdepth_tot,
                    newpair.avgb = avgb)],
            by.x = c("seqnames", "start", "end"),
...
ndt <- names(dtc6)  
ndt <- sub("newpair",names(tumorpair.dt_list[3]),ndt)
names(dtc6) <- ndt
rm(dtc5)
...
dtc7 <- merge(
            dtc6,
            tumorpair.dt_list[[4]][, .(seqnames, start, end, 
                    newpair.pwd = pwd, 
                    newpair.avgdepth_tot = avgdepth_tot,
                    newpair.avgb = avgb)],
            by.x = c("seqnames", "start", "end"),
...
ndt <- names(dtc7)  
ndt <- sub("newpair",names(tumorpair.dt_list[4]),ndt)
names(dtc7) <- ndt
rm(dtc6)
```

`r nrow(dtc7)` CpGs are retained after merging 7 PWD data sets. 
