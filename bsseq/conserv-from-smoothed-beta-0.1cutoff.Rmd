...
#  window_size <- 500
        ...
        s1.gr <- read_bedfile(sample=s1, keepchr = chr)
        s1_smoothed_beta <- calc_smoothed_beta(s1.gr,chr=chr,window_size=window_size)
 
        s2.gr <- read_bedfile(sample=s2, keepchr = chr)
...
### Effect of PWD cutoff on Number of intervals
```{r}
TumorM.dt.05<- find_Intervals_of_conserv(dtc,pwd = "MA_v_MB.pwd",pwd.consv.cutoff = 0.05)
TumorM.dt.1<- find_Intervals_of_conserv(dtc,pwd = "MA_v_MB.pwd",pwd.consv.cutoff = 0.1)

nn.dt.05<- find_Intervals_of_conserv(dtc,pwd = "JN_v_IN.pwd",pwd.consv.cutoff = 0.05)
nn.dt.1<- find_Intervals_of_conserv(dtc,pwd = "JN_v_IN.pwd",pwd.consv.cutoff = 0.1)
```
...
Fewer intervals are found by the large PWD before filtering, suggesting that intervals from the strict PWD cutoff definition are merged. We also see a decreased number after filtering. These numbers are similar between the tumor and normal tissue. Does this mean the tumor is young and lacks PWD aging?  We can look at Tumor E for larger PWDs, but do those reflect conservation or are the data noisy?

For this one, I like that Tumor M has more conserved regions than normal tissue under strict PWD, which reflects good conservation in paired sample. I also like that for the more relaxed PWD it has fewer conserved regions than normal, maybe reflecting cancer heterogeneity?
...
```{r mostCpGs}
temp <- TumorM.dt[ n_CpG == max(n_CpG),]
temp
```
...
```{r plot1-region1}
plot_region(pos1 = temp$start, posn = temp$end)
```

This data looks good. The methylation is conserved in tumor.

What about the widest interval?
```{r widest}
temp <- TumorM.dt[ width == max(width),]
temp
```
...
Should I add another filter? Re-clustering does not pick more outliers, but splits the intervals into 2 groups. If I require >9 CpGs (instead of 4+), I can drop 25\% of intervals (~20,000 remaining).  Let's just give these intervals to Darryl, and we can decide later how to fine tune the list.

## Output 1Mb regions for Conserved regions
...
```{r, warning=FALSE}
TumorM.gr_big <- resize(TumorM.gr, 1000000, fix = "center")
nn.gr_big <- resize(nn.gr, 1000000, fix = "center")
...
Does this cover the entire chromosome?
```{r}
reduce_nnbig <- reduce(nn.gr_big)
reduce_nnbig
```

According to ChatGPT the centromere spans:
Start: 23,610,000
End: 24,301,000


```{r output}
rtracklayer::export(nn.gr_big, "data/chr17consvIn-JNIN.bed", format = "BED")
rtracklayer::export(TumorM.gr_big, "data/chr17consvIn-MAMB.bed", format = "BED")

nn.dt_big <- as.data.table(nn.gr_big)
TumorM.dt_big <- as.data.table(TumorM.gr_big)

wb <- createWorkbook()
addWorksheet(wb, "JN_v_IN")
addWorksheet(wb, "Tumor_MA_v_MB")

writeData(wb, "JN_v_IN", nn.dt_big)
writeData(wb, "Tumor_MA_v_MB", TumorM.dt_big)

saveWorkbook(wb, "data/Chr17ConservedIntervals.xlsx", overwrite = TRUE)
```
